...
def valid_or_not(f):
	def func(*args, **kwargs):
		for arg in args:
			if reg.search(arg) is not None:
				log_string(args[0],"{} <-- contains illegal character".format(arg),"Red")
				return False
...
def create_dummy_machine(uuid,box):
	try:
		...
		machine = VirtualBox().create_machine(settings_file="",name=box["vm"],os_type_id="",groups=['/'],flags ="")
		VirtualBox().register_machine(machine)
		virtual_machine = VirtualBox().find_machine(box["vm"])
		with virtual_machine.create_session() as session:
			process, x = session.machine.take_snapshot(box["snapshot"], datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"),True)
...
def get_machine_status(uuid,box):
	try:
		virtual_machine = VirtualBox().find_machine(box["vm"])
		log_string(uuid,"machine status is {}".format(virtual_machine.state),"Red")
		return True
	except:
...
def test_dummy(uuid,box):
	ret = False
	if not check_if_exists_machines(uuid,box):
		create_dummy_machine(uuid,box)
	if turn_off(uuid,box,5):
		if restore_machine(uuid,box):
			if get_machine_status(uuid,box):
				if box["vm"] == "dummy":
					if delete_machine(uuid,box):
						log_string(uuid,"{} Passed, everything looks good!".format(box["vm"]),"Green")
				ret = True
	if ret: 
		exit(True)
	else:
		if box["vm"] == "dummy":
			delete_machine(uuid,box)
		exit(False)
...
def test_dummy_in_process(uuid,box,process_wait_time):
	...
	with kill_vm_and_keys_on_enter_and_exit(uuid,box):
		...
		if p.is_alive():
			...
			if p.exitcode == True:
				log_string(uuid,"test_dummy_in_process finished successfully","Green")
				ret = True
			else:
				log_string(uuid,"test_dummy_in_process finished with errors, exit code False","Red")
	return ret
